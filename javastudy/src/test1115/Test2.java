...
public class Test2 {
	public static void main(String[] args) throws IOException {
		...
		for(String f : f1.list()) {
			...
			if(f2.isFile() && f2.getName().indexOf("InputStream") >= 0 && f2.getName().substring(f2.getName().lastIndexOf('.'), f2.getName().length()).equals(".java")) {
				// System.out.println(f2.getName());
				//FileInputStream fis = new FileInputStream(orgPath+f2.getName());
				//FileOutputStream fos = new FileOutputStream(copyPath+saveFileName, true);
				
				//byte[] buf = new byte[fis.available()];
//				while((len = fis.read(buf)) != -1) {
//					fw.write .write(buf, 0, len);
//				}
